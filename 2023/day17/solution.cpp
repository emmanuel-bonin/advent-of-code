...
struct State {
    ...
    bool operator==(const State& state) const {
        return state.position == position && state.moves_in_dir == moves_in_dir &&
...
std::ostream& operator<<(std::ostream& os, const State& s) {
    ...
    os << "[" << s.position.row << "," << s.position.col << "]: Direction " << direction << ' '
       << s.heat_loss;
    return os;
}
...
int main(int argc, char* argv[]) {
    ...
    motions.push_back(Point(-1, 0));
    motions.push_back(Point(0, 1));
    motions.push_back(Point(1, 0));
    motions.push_back(Point(0, -1));
    ...
    while(!pq.empty()) {
        ...
        // Move left
        {
            ...
            if(in_limits(map, new_state.position.row, new_state.position.col)) {
                new_state.heat_loss =
                    state.heat_loss + map[new_state.position.row][new_state.position.col];
