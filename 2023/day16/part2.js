...
class Grid {
  ...
  reset() {
    for (let y = 0; y < this.sizeY; y++) {
      for (let x = 0; x < this.sizeX; x++) {
        if (this.tiles[y][x].isEnergized()) {
          this.tiles[y][x].incomingBeams = []
          this.tiles[y][x].outgoingBeams = []
        }
      }
    }
  }
...
for (let i = 0; i < grid.sizeX; i++) {
  grid.reset()
  grid.compute(i, 0, 'down')
  let tmp = grid.getTotalEnergized()
  console.log(`for x = ${i}, y = 0, direction = down => ${tmp}`)
  res = Math.max(res, tmp)

  grid.reset()
  grid.compute(i, grid.sizeY - 1, 'up')
  tmp = grid.getTotalEnergized()
  console.log(`for x = ${i}, y = ${grid.sizeY - 1}, direction = up => ${tmp}`)
  res = Math.max(res, tmp)
}
for (let i = 0; i < grid.sizeY; i++) {
  grid.reset()
  grid.compute(0, i, 'right')
  let tmp = grid.getTotalEnergized()
  console.log(`for x = 0, y = ${i}, direction = right => ${tmp}`)
  res = Math.max(res, tmp)

  grid.reset()
  grid.compute(grid.sizeX - 1, i, 'left')
  tmp = grid.getTotalEnergized()
  console.log(`for x = ${grid.sizeX - 1}, y = ${i}, direction = left => ${tmp}`)
  res = Math.max(res, tmp)
}
